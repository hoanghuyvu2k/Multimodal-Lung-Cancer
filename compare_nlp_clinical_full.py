"""
Script so sánh đầy đủ mô hình Attention gốc và NLP Clinical Attention
trên các tổ hợp modality có chứa dữ liệu lâm sàng (cnl_dem_labs).
"""
import sys
import pandas as pd
import numpy as np
...
print("=" * 80)
print("SO SÁNH ĐẦY ĐỦ MÔ HÌNH ATTENTION GỐC VÀ NLP CLINICAL EMBEDDING")
print("=" * 80)
...
for idx, case in enumerate(valid_test_cases):
    ...
    try:
        ...
        # CHẠY MÔ HÌNH GỐC
        print(f"   - Đang chạy mô hình gốc (Tham số số học)...")
        ...
        # CHẠY MÔ HÌNH NLP
        print(f"   - Đang chạy mô hình NLP (Text Embedding)...")
        ...
        summary_nlp, _ = train(
            ...
            outcomes=labels_nlp,
            l1_dfs_filter=case['filters'], # NLP module is usually index matched or not included in rad filters so it's fine
        ...
        results_summary.append({
            ...
            'Original_CI_Lower': ci_orig[0],
            'Original_CI_Upper': ci_orig[1],
            'NLP_AUC': auc_nlp,
            'NLP_CI_Lower': ci_nlp[0],
            'NLP_CI_Upper': ci_nlp[1],
            'AUC_Difference': improvement,
            'AUC_Improvement_%': improvement_pct,
            'Better_Model': 'NLP' if improvement > 0 else 'Original' if improvement < 0 else 'Equal'
        })
        
        print(f"   ✓ Original AUC: {auc_orig:.4f}")
        print(f"   ✓ NLP AUC:      {auc_nlp:.4f}")
...
print("\nBẢNG SO SÁNH CHI TIẾT CHO CÁC TỔ HỢP CÓ CLINICAL DATA\n")
print(df_comparison.to_string(index=False))

nlp_wins = sum(1 for r in results_summary if r['Better_Model'] == 'NLP')
...
print(f"  - NLP Clinical tốt hơn: {nlp_wins} ({100*nlp_wins/len(results_summary):.1f}%)")
print(f"  - Original Labs tốt hơn: {original_wins} ({100*original_wins/len(results_summary):.1f}%)")

print("\nĐang lưu kết quả Excel...")
output_dir = Path('./excel')
output_dir.mkdir(exist_ok=True)

with pd.ExcelWriter(output_dir / 'nlp_comparison_full_results.xlsx', engine='xlsxwriter') as writer:
...
print(f"✓ Hoàn tất! Đã lưu file tại {output_dir / 'nlp_comparison_full_results.xlsx'}")
